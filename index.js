...
function settings() {
  ...
  if (previousSettingsVersion !== DEFAULT_SETTINGS.settingsVersion) {
    ...
    // 0.9.7 changed the stock cumulative interval from 100 to 75, but older installations retained
    // the stored legacy value forever. Treat exactly 100 as the legacy stock value during this
    // one-time upgrade; users who deliberately prefer 100 can set it again after migration.
    if (Number(s.mergeInterval) === 100) {
      s.mergeInterval = DEFAULT_MERGE_INTERVAL;
    }
...
function relationshipInjectionBlock(data) {
  ...
  const prefix = data.processing?.relationshipDirty
    ? `（以下为已回退并验证的安全关系快照${table.lastGoodFloor >= 0 ? `，截至第 ${table.lastGoodFloor + 1} 楼` : ''}；最新待摘要楼层暂未计入，重建完成后自动更新）\n`
    : '';
  return clampTextHeadTail(`${prefix}${content}`, RELATIONSHIP_MEMORY_CHAR_BUDGET, 0.34);
}
...
async function drainDueDerivedMemory(anchorInterval, mergeInterval, maxPasses = 12) {
  let anchorsMade = 0;
  let mergesMade = 0;
...
async function runMemoryJobQueue() {
  ...
  try {
    do {
      ...
      await drainDueDerivedMemory(anchorInterval, mergeInterval, intervalRecheck ? 12 : 6);
...
function statusText(data = memoryData()) {
  ...
  const rebuildCheckpoint = data.processing?.codexRebuildCheckpoint;
  ...
  const currentMergeCycle = mergeCycleMaterials(data).length;
  const nextMergeAt = mergedCount + mergeInterval;
  const mergeRemaining = Math.max(0, mergeInterval - currentMergeCycle);
  ...
  const mergeProgress = mergeRemaining > 0
    ? `下一合并 AI回合 ${nextMergeAt}（本周期 ${currentMergeCycle}/${mergeInterval}，还差 ${mergeRemaining} 个AI回复）`
    : `全量合并已达阈值（本周期 ${currentMergeCycle}/${mergeInterval}），等待后台生成`;
  ...
  const rebuild = rebuildCheckpoint
    ? `，索引分段重建 ${Math.min(Number(rebuildCheckpoint.cursor || 0), Number(rebuildCheckpoint.totalChunks || 0))}/${Number(rebuildCheckpoint.totalChunks || 0)}`
    : '';
  const rebuildPaused = data.processing?.codexDirty
    && Number(data.processing?.codexRebuildFailures || 0) >= 3
    && Number(data.processing?.codexRetryAt || 0) > 0
      ? '，自动重试已暂停（可手动继续）'
      : '';
  const relationship = relationshipPending ? '，人物关系待重建' : '';
  return `${currentCharacterName()}：当前 AI回合 ${currentAiTurn}；${anchorProgress}；${mergeProgress}。摘要 ${readyGodlogs} 条，连续待锚定 ${continuousReady} 条，待摘要 ${pendingSummaries} 楼${codex}${rebuild}${rebuildPaused}${relationship}${queue}${lastError}`;
}
...
function updatePreview() {
  ...
  const codexStatus = data.processing?.codexDirty
    ? (codexSnapshotSafeForInjection(data)
      ? `索引待重建：主模型继续使用截至最近有效摘要的安全快照，最新待摘要楼层暂未计入。${data.processing.codexDirtyReason ? ` 原因：${data.processing.codexDirtyReason}` : ''}`
...
function renderHealth() {
  ...
  if (data.processing?.codexDirty) {
    issues.push(codexSnapshotSafeForInjection(data)
      ? `人物/物品/场景索引需要重建；当前仍注入截至最近有效摘要的安全快照，最新待摘要楼层暂不计入。${data.processing.codexDirtyReason ? `原因：${data.processing.codexDirtyReason}` : ''}`
...
function renderRelationshipEditor(data = memoryData()) {
  ...
  const status = data.processing?.relationshipDirty
    ? `关系表待重建：${data.processing.relationshipDirtyReason || '固定名单或剧情来源已变化'}。固定行不会丢失；主模型会继续使用已回退的安全快照，最新待摘要楼层暂不计入。`
